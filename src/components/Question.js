...
export const Question = () => {
  ...
  const { quizes, clearSearchQuery } = useContext(QuizContext);
  const { id } = useParams();
      ...
      <div className="flex flex-col justify-center items-center">
        ...
        <div className="mt-5 md:grid md:grid-cols-2 gap-20">
          {questions[index].options.map((option) => (
            <div
              onClick={() => chooseAnswer(option.isCorrect)}
              key={option.id}
              className="flex mb-6 w-full cursor-pointer hover:scale-105 duration-150"
            >
              <div className="bg-gray-300 h-100 rounded-l-lg p-4 flex items-center">
                {option.label}
              </div>
              <p
                className={`rounded-r-lg text-center p-5 w-full ${
